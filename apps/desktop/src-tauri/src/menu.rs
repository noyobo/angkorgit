...
pub fn build_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Menu<R>> {
    ...
    if IS_MACOS {
        ...
        let quit_item = MenuItem::with_id(app, "quit", "Quit AngKorGit", true, Some("CmdOrCtrl+Q"))?;
    ...
    // File menu
    let open_repo = MenuItem::with_id(app, "open-repo", if IS_MACOS { "Open Repository…" } else { "&Open repository…" }, true, Some("CmdOrCtrl+O"))?;
    let mut file_menu = SubmenuBuilder::new(app, if IS_MACOS { "File" } else { "&File" })
        .item(&open_repo)
        .separator()
        .text("clone-repo", if IS_MACOS { "Clone Repository…" } else { "Clo&ne repository…" });

    if !IS_MACOS {
        let settings_item = MenuItem::with_id(app, "settings", "&Options…", true, Some("CmdOrCtrl+,"))?;
    ...
    // Edit menu
    let find_item = MenuItem::with_id(app, "find", if IS_MACOS { "Find" } else { "&Find" }, true, Some("CmdOrCtrl+F"))?;
    ...
    let zoom_in = MenuItem::with_id(app, "zoom-in", if IS_MACOS { "Zoom In" } else { "Zoom in" }, true, Some("CmdOrCtrl+="))?;
    let zoom_out = MenuItem::with_id(app, "zoom-out", if IS_MACOS { "Zoom Out" } else { "Zoom out" }, true, Some("CmdOrCtrl+-"))?;
    let zoom_reset = MenuItem::with_id(app, "zoom-reset", if IS_MACOS { "Reset Zoom" } else { "Reset zoom" }, true, Some("CmdOrCtrl+0"))?;
    let dev_tools = MenuItem::with_id(app, "dev-tools", if IS_MACOS { "Toggle Developer Tools" } else { "Toggle &developer tools" }, true, Some(if IS_MACOS { "Alt+Cmd+I" } else { "Ctrl+Shift+I" }))?;
    ...
    let view_menu = SubmenuBuilder::new(app, if IS_MACOS { "View" } else { "&View" })
        ...
        .text("show-changes", if IS_MACOS { "Show Working Copy" } else { "&Working copy" })
        .text("show-history", if IS_MACOS { "Show Commit Graph" } else { "&Commit graph" })
        .separator()
        .item(&show_tabs)
        .item(&show_branches)
        .text("show-worktrees", if IS_MACOS { "Show Worktrees List" } else { "Wor&ktrees list" })
        .separator()
        .item(&go_summary)
        .item(&toggle_sidebar)
    ...
    let repo_menu = SubmenuBuilder::new(app, if IS_MACOS { "Repository" } else { "&Repository" })
        ...
        .item(&view_forge)
        .separator()
        .text("new-worktree", if IS_MACOS { "New Worktree…" } else { "New work&tree…" })
        .separator()
        .item(&refresh_item)
        .item(&close_all_tabs)
        .build()?;
    ...
    // Branch menu
    let new_branch = MenuItem::with_id(app, "new-branch", if IS_MACOS { "New Branch…" } else { "New &branch…" }, true, Some("CmdOrCtrl+Shift+N"))?;
    let rename_branch = MenuItem::with_id(app, "rename-branch", if IS_MACOS { "Rename Branch…" } else { "&Rename branch…" }, true, Some("CmdOrCtrl+Shift+R"))?;
    let delete_branch = MenuItem::with_id(app, "delete-branch", if IS_MACOS { "Delete Branch…" } else { "&Delete branch…" }, true, Some("CmdOrCtrl+Shift+D"))?;
    let stash_changes = MenuItem::with_id(app, "stash-changes", if IS_MACOS { "Stash Changes…" } else { "&Stash changes…" }, true, Some("CmdOrCtrl+Shift+S"))?;
    let merge_branch = MenuItem::with_id(app, "merge-branch", if IS_MACOS { "Merge into Current Branch…" } else { "&Merge into current branch…" }, true, Some("CmdOrCtrl+Shift+M"))?;
    ...
    let branch_menu = SubmenuBuilder::new(app, if IS_MACOS { "Branch" } else { "&Branch" })
        ...
        .item(&delete_branch)
        .separator()
        .text("discard-all", if IS_MACOS { "Discard All Changes…" } else { "Discard all changes…" })
        .item(&stash_changes)
        .separator()
        .item(&merge_branch)
        .text("rebase-branch", if IS_MACOS { "Rebase Current Branch…" } else { "R&ebase current branch…" })
        .separator()
        .text("create-pr", if IS_MACOS { "Create Pull Request" } else { "Create &pull request" })
        .build()?;
    builder = builder.item(&branch_menu);

    // Window menu (macOS)
    if IS_MACOS {
        let close_all_tabs_win = MenuItem::with_id(app, "close-all-tabs", "Close All Tabs", true, Some("CmdOrCtrl+Shift+W"))?;
